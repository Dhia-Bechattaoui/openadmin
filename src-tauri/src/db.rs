...
pub fn init_db(app_dir: &PathBuf) -> Result<Connection> {
    ...
    conn.execute(
        "INSERT OR IGNORE INTO settings (id, notifications_enabled) VALUES (1, 1)",
        [],
    )?;
...
pub fn get_settings(conn: &Connection) -> Result<Settings> {
    let mut stmt = conn.prepare("SELECT notifications_enabled FROM settings WHERE id = 1")?;
...
pub fn update_settings(conn: &Connection, settings: Settings) -> Result<()> {
    conn.execute(
        "UPDATE settings SET notifications_enabled = ?1 WHERE id = 1",
        rusqlite::params![settings.notifications_enabled],
    )?;
    Ok(())
}
